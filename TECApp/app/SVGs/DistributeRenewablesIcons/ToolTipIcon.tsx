import React from 'react'
import { Svg, G, Path, RadialGradient, Defs, Stop, Circle } from 'react-native-svg'

type ToolTipIconProps = {
  header?: boolean
  flash?: boolean
}

export const ToolTipIcon = ({ header }: ToolTipIconProps) => (
  <>
    {!header ? (
      <Svg width="22" height="22" viewBox="0 0 18 18" fill="none">
        <Defs>
          <RadialGradient
            id="grad"
            cx={8}
            cy={8}
            rx={12}
            ry={12}
            fx={10}
            fy={10}
            gradientUnits='userSpaceOnUse'
          >
            <Stop offset="0" stopColor="#12c742" stopOpacity="0.8" />
            <Stop offset="1" stopColor="#FFF" stopOpacity="0"/>
            </RadialGradient>
        </Defs>
        <G>
          <Path
            d="M8.25 6.75H9.75V5.25H8.25M9 15C5.6925 15 3 12.3075 3 9C3 5.6925 5.6925 3 9 3C12.3075 3 15 5.6925 15 9C15 12.3075 12.3075 15 9 15ZM9 1.5C8.01509 1.5 7.03982 1.69399 6.12987 2.0709C5.21993 2.44781 4.39314 3.00026 3.6967 3.6967C2.29018 5.10322 1.5 7.01088 1.5 9C1.5 10.9891 2.29018 12.8968 3.6967 14.3033C4.39314 14.9997 5.21993 15.5522 6.12987 15.9291C7.03982 16.306 8.01509 16.5 9 16.5C10.9891 16.5 12.8968 15.7098 14.3033 14.3033C15.7098 12.8968 16.5 10.9891 16.5 9C16.5 8.01509 16.306 7.03982 15.9291 6.12987C15.5522 5.21993 14.9997 4.39314 14.3033 3.6967C13.6069 3.00026 12.7801 2.44781 11.8701 2.0709C10.9602 1.69399 9.98491 1.5 9 1.5ZM8.25 12.75H9.75V8.25H8.25V12.75Z"
            fill="#266297"
          />
          <Circle fill="url(#grad)" cx={5} cy={5} r={15}>

          </Circle>
        </G>
      </Svg>
    ) : (
      <Svg width="25" height="25" viewBox="0 0 20 20" fill="none">
        <Path
          d="M9 7H11V5H9M10 18C5.59 18 2 14.41 2 10C2 5.59 5.59 2 10 2C14.41 2 18 5.59 18 10C18 14.41 14.41 18 10 18ZM10 0C8.68678 0 7.38642 0.258658 6.17317 0.761205C4.95991 1.26375 3.85752 2.00035 2.92893 2.92893C1.05357 4.8043 0 7.34784 0 10C0 12.6522 1.05357 15.1957 2.92893 17.0711C3.85752 17.9997 4.95991 18.7362 6.17317 19.2388C7.38642 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7413 7.38642 19.2388 6.17317C18.7362 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0ZM9 15H11V9H9V15Z"
          fill="#266297"
        />        
      </Svg>
    )}
  </>
)
